...
def TGSTicket2hashcat(res):	
	...
	if tgs_encryption_type == 23:
		...
		return f'$krb5tgs${tgs_encryption_type}$*{tgs_name_string}${tgs_realm}$spn*${tgs_checksum.hex()}${tgs_encrypted_data2.hex()}'
		...
		return f'$krb5tgs${tgs_encryption_type}${tgs_name_string}${tgs_realm}${tgs_checksum.hex()}${tgs_encrypted_data2.hex()}'


	else:
		return f'unknown$enctype${tgs_encryption_type}'
...
def TGTTicket2hashcat(res):
	...
	return f'$krb5asrep${tgt_encryption_type}${tgt_name_string}@{tgt_realm}:{tgt_checksum.hex()}${tgt_encrypted_data2.hex()}'

def tgt_to_kirbi(tgs, encTGSRepPart):
	#encTGSRepPart is already decrypted at this point
	ci = {
		'key': encTGSRepPart['key'],
		'prealm': tgs['crealm'],
		'pname': tgs['cname'],
		'flags': encTGSRepPart['flags'],
		'authtime': encTGSRepPart['authtime'],
		'starttime': encTGSRepPart['starttime'],
		'endtime': encTGSRepPart['endtime'],
		'renew-till': encTGSRepPart['renew-till'],
		'srealm': encTGSRepPart['srealm'],
		'sname': encTGSRepPart['sname'],
	}

	ti = {'ticket-info': [KrbCredInfo(ci)]}
	te = {'etype': 0, 'cipher': EncKrbCredPart(ti).dump()}
	t = {'pvno': 5, 'msg-type': 22, 'enc-part': EncryptedData(te)}
